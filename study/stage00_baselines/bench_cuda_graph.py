"""Controlled single-GPU CUDA Graph experiment; run each mode in a new process."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    try:
        for iteration in range(4):
            ...
            if iteration:
                runs.append(record)
        result = dict(graph_max_bs=args.graph, overlap=False, cache='naive', requests=args.requests,
        ...
        Path(__file__).with_name(f'cuda_graph_{args.graph}{suffix}.json').write_text(json.dumps(result, indent=2) + '\n')
    finally:
        llm.shutdown()
