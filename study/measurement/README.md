...
One warmup is excluded. Five traced and five untraced trials alternate order. Both modes collect outputs and perform completion checks, so overhead measures extra event recording, not the full harness overhead versus a server. Every request must return its specified length; output hashes must match; cache integrity and table-slot recovery are checked after each trial. Unit tests cover known metrics, absent results, invalid ordering, single-token output and duplicate completion.
